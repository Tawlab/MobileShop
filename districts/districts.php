...
if (isset($_GET['add_success']) && $_GET['add_success'] == 'true') {
  $alert_message = 'เพิ่มข้อมูลอำเภอใหม่สำเร็จ';
  $alert_type = 'success';
} elseif (isset($_GET['edit_success']) && $_GET['edit_success'] == 'true') {
  $alert_message = 'แก้ไขข้อมูลอำเภอสำเร็จ';
  $alert_type = 'success';
} elseif (isset($_GET['delete_success']) && $_GET['delete_success'] == '1') {
  $alert_message = 'ลบข้อมูลอำเภอสำเร็จ';
  $alert_type = 'success';
} elseif (isset($_GET['error'])) {
  if ($_GET['error'] == 'has_subdistricts') {
    $alert_message = 'ไม่สามารถลบอำเภอได้ เนื่องจากมีข้อมูลตำบลอ้างอิงอยู่';
  } elseif ($_GET['error'] == 'not_found') {
    $alert_message = 'ไม่พบข้อมูลอำเภอที่ต้องการ';
  } else {
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        @media (max-width: 767.98px) {
            ...
            /* **[เพิ่ม]** เผื่อกรณีที่มี Filter/Action Button อยู่ด้านบนตาราง */
            .row > [class*='col-'] {
                 margin-bottom: 10px;
            }
        }
        ...
        <div class="container py-4">
          ...
          <div class="card shadow-sm rounded-4 p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
              <h4 class="m-0 text-success"><i class="bi bi-building me-2"></i>รายการอำเภอ</h4>
              <a href="add_districts.php" class="btn btn-success">
              ...
              <table class="table table-hover align-middle text-center">
                <thead class="align-middle">
                  <tr>
                    ...
                    <th style="width: 25%;">ชื่ออำเภอ (ไทย)</th>
                    <th style="width: 25%;">ชื่ออำเภอ (อังกฤษ)</th>
                ...
                <tbody>
                  ...
                  while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td class="text-start"><?= htmlspecialchars($row['district_name_th']) ?></td>
                      <td class="text-start"><?= htmlspecialchars($row['district_name_en']) ?></td>
                      <td><?= htmlspecialchars($row['province_name_th']) ?></td>
                      <td>
                        <a href="edit_districts.php?id=<?= $row['district_id'] ?>" class="btn btn-outline-warning btn-sm me-1" title="แก้ไข">
                          <i class="bi bi-pencil-square"></i>
                        </a>
                        <button class="btn btn-outline-danger btn-sm delete-btn"
                          data-id="<?= $row['district_id'] ?>"
                          data-name="<?= htmlspecialchars($row['district_name_th']) ?>"
                          title="ลบ">
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title text-danger"><i class="bi bi-exclamation-triangle-fill me-2"></i>ยืนยันการลบอำเภอ</h5>
