...
if (isset($_GET['ajax'])) {
    ...
    if (!empty($search)) {
        $conditions[] = "(d.district_name_th LIKE '%$search%' OR d.district_name_en LIKE '%$search%' OR p.province_name_th LIKE '%$search%')";
    }
    $where_sql = !empty($conditions) ? "WHERE " . implode(" AND ", $conditions) : "";

    // นับจำนวนหน้าทั้งหมด
    $count_res = $conn->query("SELECT COUNT(*) as total FROM districts d LEFT JOIN provinces p ON d.provinces_province_id = p.province_id $where_sql");
    $total_items = $count_res->fetch_assoc()['total'];
    $total_pages = ceil($total_items / $limit);

    // ดึงข้อมูลหลัก
    $sql = "SELECT d.*, p.province_name_th, s.shop_name 
            FROM districts d 
            LEFT JOIN provinces p ON d.provinces_province_id = p.province_id 
            LEFT JOIN shop_info s ON d.shop_info_shop_id = s.shop_id 
            $where_sql 
            ORDER BY d.shop_info_shop_id ASC, p.province_name_th ASC, d.district_id ASC 
        ...
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    ...
                    <th width="10%">รหัส</th>
                    <th width="20%">ชื่ออำเภอ (ไทย)</th>
                    <th width="20%">ชื่ออำเภอ (อังกฤษ)</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center small"><span class="badge bg-light text-dark border">#<?= $row['district_id'] ?></span></td>
                    <td class="fw-bold"><?= htmlspecialchars($row['district_name_th']) ?></td>
                    <td class="text-muted small"><?= htmlspecialchars($row['district_name_en'] ?? '-') ?></td>
                    <td><span class="text-dark small"><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($row['province_name_th']) ?></span></td>
                    ...
                    <td class="text-center">
                        <?php if ($is_super_admin || $row['shop_info_shop_id'] == $shop_id): ?>
                            <div class="d-flex justify-content-center gap-2">
                                <a href="edit_districts.php?id=<?= $row['district_id'] ?>" class="btn btn-outline-warning btn-sm border-0"><i class="bi bi-pencil-square fs-5"></i></a>
                                <button onclick="confirmDelete(<?= $row['district_id'] ?>, '<?= addslashes($row['district_name_th']) ?>')" class="btn btn-outline-danger btn-sm border-0"><i class="bi bi-trash3-fill fs-5"></i></button>
                            </div>
                        <?php else: ?>
                ...
                <?php endwhile; ?>
                <?php else: ?>
                <tr><td colspan="7" class="text-center py-5 text-muted italic">-- ไม่พบข้อมูลอำเภอ --</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<head>
    <meta charset="UTF-8">
    <title>จัดการอำเภอ - Mobile Shop</title>
...
<body>
                ...
                <div class="container py-2" style="max-width: 1200px;">
                    <div class="main-card card">
                        <div class="card-header-custom d-flex justify-content-between align-items-center">
                            <h4><i class="bi bi-building me-2"></i>รายการข้อมูลอำเภอ</h4>
                            <a href="add_districts.php" class="btn btn-light btn-sm fw-bold">
                                <i class="bi bi-plus-circle me-1"></i> เพิ่มอำเภอ
                            </a>
                        </div>
                            ...
                            <div class="row mb-4">
                                <div class="col-md-6 col-lg-5">
                                    <div class="input-group">
                                        ...
                                        <input type="text" id="searchInput" class="form-control border-start-0" placeholder="ค้นหาชื่ออำเภอ หรือชื่อจังหวัด...">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body text-center py-4">
                    <p class="fs-5 mb-1">คุณแน่ใจหรือไม่ที่จะลบอำเภอ <strong id="delName"></strong> ?</p>
                    <p class="text-danger small mb-0">การลบอาจส่งผลต่อข้อมูลที่เกี่ยวข้องในระบบ</p>
                </div>
    ...
    <script>
        function fetchDistrictData(page = 1, search = '') {
            const container = document.getElementById('tableContainer');
            fetch(`districts.php?ajax=1&page=${page}&search=${encodeURIComponent(search)}`)
                .then(res => res.text())
        ...
        document.addEventListener('click', e => {
            if (e.target.classList.contains('ajax-page-link') || e.target.closest('.ajax-page-link')) {
                ...
                fetchDistrictData(link.dataset.page, document.getElementById('searchInput').value);
            }
            if (e.target.id === 'btnJumpPage' || e.target.closest('#btnJumpPage')) {
                ...
                if (targetPage >= 1 && targetPage <= maxPage) {
                    fetchDistrictData(targetPage, document.getElementById('searchInput').value);
                } else {
        ...
        function confirmDelete(id, name) {
            ...
            document.getElementById('confirmDelBtn').href = `delete_districts.php?id=${id}`;
            new bootstrap.Modal(document.getElementById('deleteModal')).show();
        }

        window.onload = () => fetchDistrictData();
    </script>
</body>
</html>