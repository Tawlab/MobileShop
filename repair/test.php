...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_item'])) {
    ...
    if (!empty($item_value)) {
        ...
        if ($type === 'stock') {
            ...
            $stmt = $conn->prepare("INSERT INTO bill_details (amount, price, bill_headers_bill_id, products_prod_id, prod_stocks_stock_id, create_at, update_at) VALUES (1, ?, ?, ?, ?, NOW(), NOW())");
            $stmt->bind_param("diii", $price, $bill_id, $prod_id, $stock_id);
            ...
            $prod_id = $id; 
            $stmt = $conn->prepare("INSERT INTO bill_details (amount, price, bill_headers_bill_id, products_prod_id, prod_stocks_stock_id, create_at, update_at) VALUES (1, ?, ?, ?, NULL, NOW(), NOW())");
            $stmt->bind_param("dii", $price, $bill_id, $prod_id);
            $stmt->execute();
        }
                                                ...
                                                ?>
                                                        <tr>
                                                            <td class="ps-3">
                                                                ...
                                                                <?php else: ?>
                                                                    <span class="badge bg-info text-dark" style="font-size: 0.7em; margin-left: 5px;">Service</span>
                                                                <?php endif; ?>
                                                            </td>
                                ...
                                <div class="card-footer bg-white p-3 d-flex justify-content-between align-items-center">
                                    ...
                                    <?php if ($repair['bill_status'] == 'Completed'): ?>
                                        <a href="view_repair.php?id=<?= $repair_id ?>" class="btn btn-secondary">
                                            <i class="fas fa-check-circle me-2"></i> ปิดบิลแล้ว (กลับ)
                                        </a>
                                    <?php elseif ($total > 0): ?>
