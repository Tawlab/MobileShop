...
// ฟังก์ชันสำหรับดึงรหัสถัดไป n รหัส (Run ID ต่อกันไปเลยทั้งระบบ)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($brands as $index => $brand) {
        ...
        if ($has_centralinf_permission && $is_central) {
            $save_shop_id = 0; // บันทึกเป็นส่วนกลาง
        } else {
            $save_shop_id = $shop_id; // บันทึกเป็นของร้านตัวเอง
        }
        ...
        $stmt_check_id->close();
        if ($count_id > 0) {
            $errors[] = "แถวที่ " . ($index + 1) . ": รหัสยี่ห้อ '$brand_id' ซ้ำในระบบ (โปรดรีเฟรชหน้าเพื่อรับรหัสใหม่)";
            continue;
        }
        ...
        if ($count_name > 0) {
            $errors[] = "แถวที่ " . ($index + 1) . ": ชื่อยี่ห้อ '$brand_name_th' หรือ '$brand_name_en' มีอยู่แล้วในระบบ";
            continue;
        }
