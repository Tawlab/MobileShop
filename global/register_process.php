...
try {
    ...
    if ($existing_shop_id > 0) {
        ...
        $stmt_dup->execute();
        if ($stmt_dup->get_result()->num_rows > 0) throw new Exception("ชื่อร้านค้านี้มีอยู่แล้ว");
        ...
        $sql_shop = "INSERT INTO shop_info (shop_id, shop_name, shop_phone, shop_email, tax_id, Addresses_address_id, create_at, update_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())";
        $stmt_shop = $conn->prepare($sql_shop);
        $stmt_shop->bind_param("issssi", $target_shop_id, $shop_name, $shop_phone, $shop_email, $shop_tax_id, $new_address_id);
    ...
    $sql_emp = "INSERT INTO employees (emp_id, emp_code, emp_national_id, firstname_th, lastname_th, firstname_en, lastname_en, 
                emp_phone_no, emp_email, prefixs_prefix_id, Addresses_address_id, religions_religion_id, 
                departments_dept_id, branches_branch_id, users_user_id, emp_gender, emp_status) 
