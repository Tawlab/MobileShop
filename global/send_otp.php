...
// รองรับคีย์หลายรูปแบบ (emp_email, email, cs_email)
$email = $data['emp_email'] ?? $data['email'] ?? $data['cs_email'] ?? $data['supplier_email'] ?? ''; 
...
if (!$shop_info || empty($shop_info['shop_email']) || empty($shop_info['shop_app_password'])) {
    echo json_encode(['status' => 'error', 'message' => 'ระบบส่วนกลางยังไม่ได้ตั้งค่าอีเมล กรุณาติดต่อผู้ดูแลระบบ']);
    exit;
}
